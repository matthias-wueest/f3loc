...
if (net_type == "d") & ((dataset == "hge_customized_cropped") or (dataset =="hge_customized_complete")):
    ...
    n1, n2, n3 = 325907, 15057, 2746
    w1dot, w2dot, w3dot = 1/n1, 1/n2, 1/n3
    ce_weights = torch.tensor(np.array([w1dot, w2dot, w3dot])/(w1dot + w2dot + w3dot)).to(device)
...
# Define data
train_dataset = GridSeqDataset_hge_customized_cropped_gravity_align_depthanything_semantics(
    dataset_dir,
    split.train,
    L=L,
    depth_dir=depth_dir,
...
val_dataset = GridSeqDataset_hge_customized_cropped_gravity_align_depthanything_semantics(
    dataset_dir,
    split.val,
    L=L,
    depth_dir=depth_dir,
...
model = depth_net_depthanything_uncertainty_sem_pl(shape_loss_weight=shape_loss_weight, lambda_s=lambda_s, temperature = temperature, ce_weights=ce_weights, lr=lr, d_min=d_min, d_max=d_max, d_hyp=d_hyp, D=D)
#model = semantic_net_depthanything_pl(ce_weights=ce_weights, lr=lr)
