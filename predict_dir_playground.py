...
if __name__ == "__main__":
    ...
    #config = "/metric3d/mono/configs/HourglassDecoder/vit.raft5.large.py"
    config = "/cluster/home/wueestm/f3loc/metric3d/mono/configs/HourglassDecoder/vit.raft5.large.py"
    ckpt = "/cluster/home/wueestm/f3loc/metric3d/metric_depth_vit_large_800k.pth"
    log_path = "/cluster/home/wueestm/f3loc/metric3d/logs"
    fx = 1596
    fy = 1596
    cx = 960
    cy = 720
    indir = "/cluster/home/wueestm/f3loc/metric3d/data/hge_customized_complete/non-aligned/rgb"
    outdir = "/cluster/home/wueestm/f3loc/metric3d/output"
    ...
    log_fname = osp.join(log_path, 'predict.log')

    model = torch.nn.DataParallel(model).to(device)
    model, _,  _, _ = load_ckpt(ckpt, model, strict_match=False)
    model.eval()
    logger = setup_logger(log_fname)
    test_data = load_data(indir, fx, fy, cx, cy)
    cfg.show_dir = outdir
    ...
    for i, an in enumerate(test_data):
        ...
        #torch.save(pred_depth.detach().cpu().numpy(),  os.path.join(outdir, base_fn + '.pt'))
        ...
        plt.savefig(os.path.join(outdir, base_fn + '.png'), bbox_inches='tight', pad_inches=0, dpi=300)  # Save the image with high resolution
