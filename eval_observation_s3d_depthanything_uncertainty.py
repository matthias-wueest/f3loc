...
#    )
    ...
    #checkpoint_path="/cluster/home/wueestm/f3loc/tb_logs/my_model/version_239/checkpoints/epoch=18-step=154470.ckpt"
    checkpoint_path="/cluster/home/wueestm/f3loc/tb_logs/my_model/version_239/checkpoints/last.ckpt"
...
####
        ...
        if net_type == "mvd" or (net_type == "comp_s" and use_mv):
            ...
            #pred_depths = pred_depths.squeeze(0).detach().cpu().numpy()
            ### 
            loc, scale, attn, prob = d_net.encoder(ref_img_torch, ref_mask_torch)
        ...
        ### 
        pred_rays, scales_rays = get_ray_from_depth_uncertainty(pred_depths, pred_scales, V=V, dv=dv, F_W=F_W)
        ...
        #)
        ###
        prob_vol_pred, prob_dist_pred, orientations_pred, pose_pred = localize_uncertainty(
            torch.tensor(desdf["desdf"]).to(device),
            pred_rays.to(device),
            scales_rays.to(device),
            return_np=False,
            orn_slice=orn_slice
        )
        ###
