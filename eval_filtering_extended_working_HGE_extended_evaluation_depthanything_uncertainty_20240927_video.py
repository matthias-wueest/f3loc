...
suffix = "_laplace_video" # "_with_scale"
...
if dataset == "gibson_t":
    ...
    if model == "gt":
        ...
        evol_path = "./evol_path/hge_customized_cropped/depthanything_0" + str(int(desdf_resolution*10)) + "_" + str(orn_slice) + "_traj_len_" + str(traj_len) + "_dt_" + str(dt) + suffix
...
#for data_idx in tqdm.tqdm(range(5,len(test_set))):
    ...
    euler_angles = torch.tensor(data["euler_angles"], device=device).unsqueeze(0)
    print("tensor poses")
    ...
    for t in range(traj_l - L):
        ...
        if net_type == "mvd" or (net_type == "comp_s" and use_mv):
            ...
            ### Trained model:
                ...
                euler_angles_np = input_dict["euler"].squeeze(0).detach().cpu().numpy()
        ...
        if (evol_path is not None) & (log_extended==True):
            ...
            predicted_uncertainties = pred_scales
            ...
            predicted_uncertainties_ls.append(predicted_uncertainties)
            eulerangles_ls.append(euler_angles_np)
...
#                minlength=0.1,
#            )
            # Red arrow with a white outline
            ax.quiver(
                gt_pose_desdf[t + L, 0],
                gt_pose_desdf[t + L, 1],
                np.cos(gt_pose_desdf[t + L, 2]),
                np.sin(gt_pose_desdf[t + L, 2]),
                color="red",  # Main red arrow
                width=0.04,  # Thinner width for the main arrow
                scale_units="inches",
                units="inches",
                scale=5,
                headwidth=3,
            ...
            # Blue arrow with a white outline
            ax.quiver(
                pose[0],
                pose[1],
                np.cos(pose[2]),
                np.sin(pose[2]),
                color="blue",  # Main blue arrow
                width=0.04,  # Thinner width for the main arrow
                scale_units="inches",
                units="inches",
                scale=5,
            ...
            fig.savefig(
                os.path.join(
                    evol_path, "pretty_filter", str(data_idx), str(t) + ".jpg"
                ), dpi=300
            )
...
#            fig.savefig(os.path.join(evol_path, "pretty_filter_extended", str(data_idx), "bugfixing" + ".png"))
    ...
    if log_error:
        ...
        predicted_uncertainties_ls_ls.append(predicted_uncertainties_ls)
        eulerangles_ls_ls.append(eulerangles_ls)
...
if save_results:
    ...
    results_dict["per_frame"]["predicted_uncertainties_ls_ls"] = predicted_uncertainties_ls_ls
    ...
    results_dict["per_frame"]["eulerangles_ls_ls"] = eulerangles_ls_ls
